...
export class ClientWorld implements IWorld {
  ...
  private applySnapshot(snap: any): void {
    ...
    if (s && e) {
      ...
      // Crit/haste RATING are informational paper-doll stats (the combat values ride
      // crit/sh above); sent always, like the other self stats, so the online character
      // sheet shows them instead of 0. Recomputed server-side, never on the client.
